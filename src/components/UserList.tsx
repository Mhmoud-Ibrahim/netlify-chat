...
//   // 1. أولاً: نجهز القائمة ونضيف لكل مستخدم حالة الاتصال بشكل مؤقت للترتيب
//   const sortedUsers = filteredUsers?.map(user => {
//     const isOnline = onlineUsers?.some(online =>
//       String(online.userId).replace(/['"]+/g, '') === String(user._id || user.id).replace(/['"]+/g, '')
//     );
//     return { ...user, isOnline }; // ندمج حالة الاتصال مع بيانات المستخدم
//   }).sort((a, b) => (b.isOnline ? 1 : 0) - (a.isOnline ? 1 : 0)); // ترتيب: المتصل (true) أولاً
...
//                 }`}
//             > */}
//               // ... باقي كود الـ Helmet والـ Card
//       <div className="list-group list-group-flush custom-scrollbar" style={{ maxHeight: 'calc(100vh - 200px)', overflowY: 'auto' }}>
//         {sortedUsers?.map((user) => {
//           const userIdStr = String(user._id || user._id).replace(/['"]+/g, '');
//           const isSelected = selectedUser === userIdStr;
//           const isOnline = user.isOnline; // القيمة التي حسبناها بالأعلى
...
//               <div className="position-relative me-3">
//                 <div className="rounded-circle overflow-hidden border border-2 border-warning shadow-sm" style={{ width: '38px', height: '38px' }}>
//                   <img src={user.fulluserImage || user.userImage || "/default.png"}
//                    className="w-100 h-100 object-fit-cover" 
//                     onError={(e) => {
//                         e.currentTarget.style.display = 'none';
//                         const parent = e.currentTarget.parentElement;
