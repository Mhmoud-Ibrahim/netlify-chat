...
export default function Register() {
  ...
  <Helmet>
    ...
    <form onSubmit={formik.handleSubmit} className="mt-5 d-flex flex-column">
        ...
        <motion.div variants={itemVariants} className="text-center m-auto w-75">
          {loading ? (
            <button disabled type='button' className='btn btn-success mt-2 w-75 py-1 mb-2'> 
              <i className='fas fa-spinner fa-spin'></i>
            </button>
          ) : (
            <button disabled={!(formik.dirty && formik.isValid)} type='submit' className='btn btn-danger mt-2 w-75 py-1 mb-2 w-75'>
              تسجيل
            </button>
          )}
