...
// //     const {
// //         sendGroupMsg, // الدالة المخصصة للمجموعات
// //         messages,
// //         userId,
// //         selectedGroup, // نعتمد هنا على الجروب المختار
// //         userGroups,
// //         socket,
// //         loading,
// //         setLoading
...
// //     // جلب بيانات الجروب الحالي المختار من المصفوفة
// //     const currentGroupData = userGroups?.find(g => String(g._id).replace(/['"]+/g, '') === String(selectedGroup).replace(/['"]+/g, ''));

// //     // التنبيهات للحذف (نفس التنسيق)
// //     const confirmClearChat = () => {
// //         toast((t) => (
// //             <div style={{ direction: 'rtl' }}>
// //                 <p className="mb-2">مسح شات المجموعة؟</p>
// //                 <button className="btn btn-danger btn-sm me-2" onClick={() => { /* يمكنك إضافة دالة حذف شات الجروب هنا */ toast.dismiss(t.id); }}>نعم</button>
// //                 <button className="btn btn-light btn-sm" onClick={() => toast.dismiss(t.id)}>إلغاء</button>
// //             </div>
// //         ));
// //     };

// //     // جلب تاريخ المجموعة فور اختيارها
// //     useEffect(() => {
...
// //     {/* هنا غيرنا العنوان ليكون "المجموعات" */}
// //     <h5 className="mb-0 fw-bold">المجموعات</h5>
// //     <i className="fa-solid fa-layer-group"></i>
// //   </div>
  
// //   {/* هنا استدعينا GroupsList بدلاً من UsersList */}
// //   <GroupsList /> 
// // </motion.div>
...
// //                                                     {item.imageUrl && <img src={item.imageUrl} className="w-100 rounded mb-2 shadow-sm" alt="group-msg" />}
...
// //                                 <h5>دردشة المجموعات</h5>
// //                                 <p className="small">اختر مجموعة من القائمة الجانبية لبدء التواصل مع أعضائها</p>
// //                             </motion.div>
// //                         </div>
// //                     )}
// //                 </div>
...
//   // جلب بيانات الجروب الحالي المختار بكل أعضائه
//   const currentGroupData = userGroups?.find(g => String(g._id).replace(/['"]+/g, '') === String(selectedGroup).replace(/['"]+/g, ''));

//   // عند تحميل الصفحة، إذا لم يكن هناك جروب مختار، نعود لقائمة الجروبات
//   useEffect(() => {
//     if (!selectedGroup) {
//         navigate("/groups");
//     } else if (socket) {
//         // الانضمام للروم فوراً عند دخول الصفحة
//         socket.emit("join_group", { roomId: selectedGroup });
//     }
//   }, [selectedGroup, socket, navigate]);
...
//         {/* Sidebar: يعرض أعضاء الجروب الحالي فقط */}
//         <motion.div 
//           initial={{ x: -50, opacity: 0 }}
//           animate={{ x: 0, opacity: 1 }}
//           className="col-md-4 col-lg-3 border-end bg-white d-none d-md-block h-100 overflow-auto shadow-sm"
//         >
//           <div className="p-3 bg-primary text-white d-flex align-items-center justify-content-between">
//             <h5 className="mb-0 fw-bold">أعضاء المجموعة</h5>
//             <i className="fa-solid fa-users"></i>
//           </div>
          
//           <div className="list-group list-group-flush">
//             {currentGroupData?.members?.map((member: any) => (
//                 <div key={member._id} className="list-group-item border-0 d-flex align-items-center py-3 px-3 border-bottom">
...
//                         {member._id === currentGroupData.admin && <small className="text-warning" style={{fontSize:'10px'}}>مسئول المجموعة</small>}
//                    </div>
//                 </div>
//             ))}
//             <div className="p-3">
//                 <button className="btn btn-outline-primary btn-sm w-100 rounded-pill" onClick={() => navigate("/groups")}>
//                     <i className="fa-solid fa-arrow-left me-1"></i> العودة للمجموعات
//                 </button>
//             </div>
//           </div>
//         </motion.div>
...
//                   </div>
//                   <div className="ms-3 text-start">
//                     <h6 className="mb-0 fw-bold">{currentGroupData?.name}</h6>
//                     <small className="text-muted">{currentGroupData?.members?.length} عضو مضاف</small>
//                   </div>
//                 </div>
//               </motion.div>
...
//                     const isMe = String(item?.senderId || item?.sender).replace(/['"]+/g, '') === myId;
//                     return (
//                       <motion.div key={item._id || index} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className={`d-flex ${isMe ? "justify-content-end" : "justify-content-start"} mb-3`}>
//                         <div className={`p-2 shadow-sm message-bubble ${isMe ? "bg-primary text-white bubble-me" : "bg-white text-dark bubble-them"}`} style={{ maxWidth: "70%", borderRadius: "15px" }}>
//                           {!isMe && <div className="fw-bold mb-1" style={{ fontSize: '10px', color: '#6610f2' }}>{item.senderName || "عضو"}</div>}
//                           {item.imageUrl && <img src={item.imageUrl} className="w-100 rounded mb-2 shadow-sm" alt="" />}
//                           <p className="mb-1 small px-1">{item.text}</p>
//                           <div className="text-end" style={{ fontSize: '9px', opacity: 0.8 }}>{new Date(item.createdAt || Date.now()).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
//                         </div>
//                       </motion.div>
//                     );
//                   })}
...
//                   <input {...formik.getFieldProps('msg')} className="form-control rounded-pill border-0 bg-light px-4 py-2" placeholder="اكتب رسالة..." autoComplete="off" value={formik.values.msg} onChange={formik.handleChange} />
...
//               {/* <div className="p-3 bg-white border-top shadow-sm z-3">
//                 <form onSubmit={formik.handleSubmit} className="d-flex align-items-center gap-2">
//                   <input type="text" name="msg" className="form-control rounded-pill border-0 bg-light px-4 py-2" placeholder="اكتب رسالة للمجموعة..." value={formik.values.msg} onChange={formik.handleChange} autoComplete="off" />
//                   <button type="submit" className="btn btn-primary rounded-circle shadow" style={{ width: '40px', height: '40px' }} disabled={isUploading}>
                ...
                <AnimatePresence initial={false}>
                  {groupMessages.length > 0 ? (
                    groupMessages.map((item, index) => {
                      const isMe = String(item?.senderId || item?.sender).replace(/['"]+/g, '') === myId;
                        const messageKey = item._id ? item._id : `temp-${index}-${item.createdAt || Date.now()}`;
                      return (
                        <motion.div
                          key={messageKey}
                          initial={{ opacity: 0, y: 15, scale: 0.95 }}
