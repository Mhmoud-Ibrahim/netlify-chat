...
export function SocketProvider({ children }: { children: ReactNode }) {
    ...
    // دالة مساعدة لتنظيف المعرفات
    const cleanId = (id: any) => String(id || "").replace(/['"]+/g, '');

    useEffect(() => {
        // الاتصال بالسوكيت مرة واحدة فقط عند توفر الـ userId
        const newSocket = io("https://m2dd-serverchatapp.hf.space", {
            withCredentials: true,
            transports: ['websocket'],
            reconnection: true,  
            query: { userId: userId },
            reconnectionAttempts: 5, // يفضل تحديدها لتقليل الضغط
            reconnectionDelay: 1000,      
            timeout: 20000,
        });
        ...
        newSocket.on("connect", () => {
            setIsConnected(true);
            setLoading(false); // إيقاف التحميل عند الاتصال
            newSocket.emit("online_users");
        ...
        newSocket.on("receive_group_msg", (data: MsgData) => {
            setMessages((prev) => [...prev, data]);
            if (cleanId(data.senderId) !== cleanId(userId)) {
        ...
        newSocket.on("private_reply", (data: MsgData) => {
            setMessages((prev) => [...prev, data]);
            const incomingSenderId = cleanId(data.senderId);
            const currentUserId = cleanId(userId);

            if (incomingSenderId !== currentUserId) {
                setOnlineUsers((currentList) => {
                    const senderObj = currentList.find(u => cleanId(u.userId) === incomingSenderId);
                    
                    toast.success(`New message from ${senderObj?.name || "Unknown"}`, {
                        icon: '💬',
                        duration: 3000,
                    });
        ...
        return () => {
            newSocket.disconnect(); // إغلاق الاتصال عند مسح المكون
        };
    }, [userId]); // يتم التنفيذ فقط إذا تغير الـ userId


    const sendMsg = (msg: MsgData) => socket?.emit("send_message", msg);
    const deleteMsg = (msg: MsgData) => socket?.emit("delete_message", { messageId: msg._id });
    ...
    const sendPrivateMsg = (msg: string, receiverId: string, imageUrl?: string) => {
        socket?.emit("private_message", { text: msg, receiverId, imageUrl });
    };
 const logout = async () => {
        try {
            await api.post('/auth/logout');
    ...
    const sendGroupMsg = (msg: string, roomId: string, imageUrl?: string) => {
        socket?.emit("group_message", { text: msg, roomId, imageUrl });
    };
   const fetchGroups = useCallback(async () => {
    try {
        const res = await api.get("/auth/groups");
    ...
    return (
        <SocketContext.Provider value={{
            userId, setUserId, socket, isConnected, messages, userName, setUsername,
            user, setUser, selectedUser, setSelectedUser, onlineUsers, notification,
            loading, setLoading, selectedUserDatafromServer, setSelecteduserDatafromServer,
            allUsers, userGroups, selectedGroup, setSelectedGroup, sendMsg, deleteMsg,
            deleteSenderMessages, sendPrivateMsg, logout, clearNotification,
            updateUserData, sendGroupMsg, fetchGroups
        }}>
            {children}
        </SocketContext.Provider>
    );
